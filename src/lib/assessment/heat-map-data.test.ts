...
describe("buildHeatMapCells", () => {
  it("always returns 6 cells in RISK_AREAS order", () => {
    const cells = buildHeatMapCells([]);
    expect(cells.length).toBe(RISK_AREAS.length);
    expect(cells.length).toBe(6);
    expect(cells.map((c) => c.pillarId)).toEqual(RISK_AREAS.map((a) => a.id));
  });
...
describe("rowSeverity", () => {
  ...
  it("returns 0 for an all-unassessed row", () => {
    const sev = rowSeverity(buildHeatMapCells([]));
