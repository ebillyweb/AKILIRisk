...
const { fakes, resolvePillarNarrativesForAssessmentSpy } = vi.hoisted(() => {
  const resolvePillarNarrativesForAssessmentSpy = vi.fn(async () => [] as string[]);
  const state = {
  ...
  return { fakes: state, resolvePillarNarrativesForAssessmentSpy };
});
...
vi.mock("@/lib/methodology/assessment-runtime", () => ({
  ...
  resolvePillarNarrativesForAssessment: (...args: unknown[]) =>
    resolvePillarNarrativesForAssessmentSpy(...args),
}));
...
beforeEach(() => {
  resolvePillarNarrativesForAssessmentSpy.mockReset();
  resolvePillarNarrativesForAssessmentSpy.mockResolvedValue([]);
...
describe("buildReportSnapshot", () => {
  ...
  it("includes pillarNarratives from snapshotted assessment runtime", async () => {
    resolvePillarNarrativesForAssessmentSpy.mockResolvedValueOnce([
    ...
    expect(resolvePillarNarrativesForAssessmentSpy).toHaveBeenCalledWith(
      "asmt-1",
      "cyber-digital",
      6.5,
      "MEDIUM",
