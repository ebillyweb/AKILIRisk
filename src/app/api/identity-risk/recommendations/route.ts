...
export async function POST(request: NextRequest) {
  try {
    ...
    // Load identity risk pillar score
    ...
    if (!pillarScore) {
      return NextResponse.json(
        { error: "Identity risk assessment not completed. Complete the assessment to generate recommendations." },
        { status: 400 }
      );
    }

    // Load assessment responses for identity risk pillar
    const responses = await prisma.assessmentResponse.findMany({
      where: {
        assessmentId,
        skipped: false,
        // Filter for identity risk questions (assuming questionId starts with 'identity-')
        questionId: { startsWith: 'identity-' },
      },
    ...
    const recommendations = await generateIdentityRecommendations(scoreResult, answers);
    ...
    console.error("Error generating identity risk recommendations:", error);
    return NextResponse.json(
...
/**
 * GET /api/identity-risk/recommendations
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!pillarScore) {
      return NextResponse.json(
        { error: "Identity risk assessment not completed" },
        { status: 404 }
      );
    }
    ...
    console.error("Error retrieving identity risk recommendations:", error);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    );
