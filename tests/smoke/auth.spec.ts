...
interface DashboardAssertion {
   ...
   * .github/workflows/smoke-tests.yml). Only password-login roles are tagged:
   * they have no external dependency, so they stay green on preview without
   * ENABLE_TEST_AUTH. The client case exercises the magic-link flow, which
   * requires ENABLE_TEST_AUTH=1 on the target deployment, so it is left out of
   * the scheduled run (still covered by the full `npm run test:e2e` suite).
   */
  smoke: boolean;
}
